...
CREATE TABLE public.languages (
	...
	direction varchar(255) NULL DEFAULT 'ltr'::character varying,
...
CREATE TABLE public.pages (
	...
	CONSTRAINT pages_parent_page_id_foreign FOREIGN KEY (parent_page_id) REFERENCES public.pages(id)
);
...
CREATE TABLE public.seo (
	...
	CONSTRAINT seo_image_foreign FOREIGN KEY (image) REFERENCES public.directus_files(id) ON DELETE SET NULL
);
...
-- public.pages_translations definition

-- Drop table

-- DROP TABLE public.pages_translations;
...
CREATE TABLE public.pages_translations (
	...
	CONSTRAINT pages_translations_languages_code_foreign FOREIGN KEY (languages_code) REFERENCES public.languages(code) ON DELETE SET NULL,
	CONSTRAINT pages_translations_pages_id_foreign FOREIGN KEY (pages_id) REFERENCES public.pages(id) ON DELETE SET NULL,
	CONSTRAINT pages_translations_seo_id_foreign FOREIGN KEY (seo_id) REFERENCES public.seo(id) ON DELETE SET NULL
);
